    ...
    : goal_{goal}, start_{start}, MODE_{mode}
{
  ...
  goal_.x_ = (MATRIX_HEIGHT - 1) - goal.y_;
  goal_.y_ = goal.x_;

  start_.x_ = (MATRIX_HEIGHT - 1) - start.y_;
  start_.y_ = start.x_;
  ...
  {
    if (themap.data.at(i) > 0)
      map_[HEIGHT - 1 - height_lim][width_lim] = true;
    else
      map_[HEIGHT - 1 - height_lim][width_lim] = false;
...
// getchar();
  ...
  else
  {
    ...
    while (!(position == start_))
    {
      ...
      map_[position.x_][position.y_] = 1;
#ifdef DEBUG
